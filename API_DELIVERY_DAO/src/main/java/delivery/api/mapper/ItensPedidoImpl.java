...
public class ItensPedidoImpl {
	...
	public void cadastrarItemPedidoDAO(final ItemPedido itemPedido){
		final SqlSession session = ConnectionFactory.getSqlSessionFactory().openSession();
		final ItemPedidoDAO itensPedidoDao = session.getMapper(ItemPedidoDAO.class);
		itensPedidoDao.cadastrarItemPedidoDAO(itemPedido);
		session.commit();
		session.close();
	}
	
	public void excluirItemPedido(final ItemPedido itemPedido){
		final SqlSession session = ConnectionFactory.getSqlSessionFactory().openSession();
		final ItemPedidoDAO itensPedidoDao = session.getMapper(ItemPedidoDAO.class);
	...
	public List<ItemPedido> getItensPedido(final String idPedido){
		final SqlSession session = ConnectionFactory.getSqlSessionFactory().openSession();
		final ItemPedidoDAO itensPedidoDao = session.getMapper(ItemPedidoDAO.class);
		final List<ItemPedido> itensPedido = itensPedidoDao.getItensPedido(idPedido);
		session.close();
		return itensPedido;
	}
